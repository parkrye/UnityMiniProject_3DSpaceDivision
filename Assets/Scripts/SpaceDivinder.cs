...
public class SpaceDivinder : MonoBehaviour
{
    ...
    public void OnSpaceSetting()
    {
        SettingSpaces();
    }

    public void OnRouteSetting()
    {
        SettingRoutes();
    }
    ...
    void SettingRoutes()
    {
        for (int i = 0; i < WholeSpaces.Count; i++)
        {
            ...
            for (int j = i + 1; j < WholeSpaces.Count; j++)
            {
                ...
                if (Physics.Raycast(WholeSpaces[i].Position, (WholeSpaces[j].Position - WholeSpaces[i].Position).normalized, Vector3.Distance(WholeSpaces[j].Position,  WholeSpaces[i].Position), LayerMask.GetMask("Obstacle")))
                {
